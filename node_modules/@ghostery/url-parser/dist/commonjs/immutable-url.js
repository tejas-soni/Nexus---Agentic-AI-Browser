...
const url_search_params_js_1 = __importStar(require("./url-search-params.js"));
const BREAK_HOST_ON = [const_js_1.CODE_FORWARD_SLASH, const_js_1.CODE_HASH, const_js_1.CODE_QUESTION_MARK];
...
class default_1 {
    ...
    get domainInfo() {
        if (!this._domainInfo) {
            this._domainInfo = (0, tldts_experimental_1.parse)(this.hostname, {
    ...
    extractKeyValues() {
        ...
        this.parsedParameters = new url_search_params_js_1.default();
        ...
        let index = start;
        if (this.href.charCodeAt(index) === const_js_1.CODE_SEMICOLON) {
            // parameter string starts here
            index = this._extractParamTuples(index + 1, end, this.parsedParameters, [const_js_1.CODE_SEMICOLON], const_js_1.CODE_EQUALS, [const_js_1.CODE_QUESTION_MARK, const_js_1.CODE_HASH]);
        }
        if (this.href.charCodeAt(index) === const_js_1.CODE_QUESTION_MARK) {
            // query string starts here
            index = this._extractParamTuples(index + 1, end, this.parsedParameters, [const_js_1.CODE_AMPERSAND, const_js_1.CODE_SEMICOLON], // allow '&' or ';' as separators
            const_js_1.CODE_EQUALS, [const_js_1.CODE_HASH]);
        }
        return this.parsedParameters;
    }
    _extractHostname(start, end) {
        ...
        if (this._href.charCodeAt(i) === const_js_1.CODE_SQUARE_BRACKET_OPEN) {
            ipv6 = true;
            for (; i <= end; i += 1) {
                const code = this._href.charCodeAt(i);
                if (code === const_js_1.CODE_SQUARE_BRACKET_CLOSE) {
                    // after closed brackets can only be ':' or '/'
                    const nextCode = this._href.charCodeAt(i + 1);
                    if (nextCode === const_js_1.CODE_COLON) {
                        portIndex = i + 1;
                        i += 1;
                        stopped = true;
                    }
                    else if (nextCode === const_js_1.CODE_FORWARD_SLASH) {
                        i += 1;
                        stopped = true;
                    }
                    else if (i !== end) {
        ...
        if (!ipv6) {
            for (; i <= end; i += 1) {
                const code = this._href.charCodeAt(i);
                if (code === const_js_1.CODE_COLON) {
                    portIndex = i;
                    stopped = true;
                    break;
                }
                else if (code === const_js_1.CODE_AT) {
        ...
        if (this.queryStartIndex > 0) {
            ...
            if (this.href.charCodeAt(index) === const_js_1.CODE_QUESTION_MARK) {
                ...
                for (; index <= end; index += 1) {
                    if (this.href.charCodeAt(index) === const_js_1.CODE_HASH) {
                        broken = true;
                        break;
                    }
                }
    ...
    _extractSearchParams() {
        ...
        if (this.href.charCodeAt(index) === const_js_1.CODE_SEMICOLON) {
            // parameter string starts here
            index = this._extractParamTuples(index + 1, end, this._parameters, [const_js_1.CODE_SEMICOLON], const_js_1.CODE_EQUALS, [const_js_1.CODE_QUESTION_MARK, const_js_1.CODE_HASH]);
        }
        if (this.href.charCodeAt(index) === const_js_1.CODE_QUESTION_MARK) {
            // query string starts here
            const searchStart = index;
            index = this._extractParamTuples(index + 1, end, this._query, [const_js_1.CODE_AMPERSAND], const_js_1.CODE_EQUALS, [const_js_1.CODE_HASH]);
    ...
    _extractParamTuples(start, end, params, separators, equals, breakCodes) {
        return (0, url_search_params_js_1.extractParams)(this.href, start, end, params, separators, equals, breakCodes);
    }
    parse(url) {
        ...
        this._parameters = new url_search_params_js_1.default();
        this._query = new url_search_params_js_1.default();
        ...
        for (index += 1; index < end; index += 1) {
            if (this._href.charCodeAt(index) !== const_js_1.CODE_FORWARD_SLASH) {
                break;
            }
            else {
                this.slashes += '/';
        ...
        if (index >= end) {
            ...
            if (this._href.charCodeAt(end) !== const_js_1.CODE_FORWARD_SLASH) {
                this._href += '/';
            }
            this._pathname = '/';
        }
            ...
            for (; index <= end; index += 1) {
                const code = this._href.charCodeAt(index);
                if (code === const_js_1.CODE_SEMICOLON && !this.parameterStartIndex) {
                    this.parameterStartIndex = index;
                }
                else if (code === const_js_1.CODE_QUESTION_MARK || code === const_js_1.CODE_HASH) {
...
//# sourceMappingURL=immutable-url.js.map